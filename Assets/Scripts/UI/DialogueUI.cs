...
public class DialogueUI : UIBase
{
    ...
    {
        var dialogueData = GameDataManager.Instance.GetData<DialogueData>(_currentDialogueId);
    ...
    {
        var dialogueData = GameDataManager.Instance.GetData<DialogueData>(dialogeId);
        if (dialogueData == null)
        {
            Debug.LogWarning($"다이얼로그 데이터가 존재하지 않습니다 {dialogueData}");
            return;
    ...
    private void SetCharacterName(string characterDataId)
    {
        ...
        if (isActive)
        {
            var characterData = GameDataManager.Instance.GetData<CharacterData>(characterDataId);
